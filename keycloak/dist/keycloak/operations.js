 ...
 */
import { KeycloakApiError } from './client.js';
import { clientsEndpoint, clientsSearchEndpoint, clientSecretEndpoint, protocolMappersEndpoint, } from './endpoints.js';
/**
...
export async function findClientByClientId(client, clientId) {
    ...
    const clients = await client.get(url);
 ...
 */
export async function createClient(client, input) {
    ...
    try {
        ...
        const secretResponse = await client.get(secretUrl);
        return {
            success: true,
            clientId: input.clientId,
            internalId,
 ...
 */
export async function getClientSecret(client, clientId) {
    ...
    const secretResponse = await client.get(url);
    return {
        success: true,
        clientId,
        secret: secretResponse.value,
 ...
 */
export async function addGroupsMapper(client, input) {
    // Find the client

...
const BROWSER_HEADLESS = process.env.PLAYWRIGHT_HEADLESS !== 'false';
const DEFAULT_TIMEOUT = parseInt(process.env.PLAYWRIGHT_TIMEOUT || '30000');
const VIEWPORT_WIDTH = parseInt(process.env.BROWSER_VIEWPORT_WIDTH || '1280');
const VIEWPORT_HEIGHT = parseInt(process.env.BROWSER_VIEWPORT_HEIGHT || '720');
...
app.post('/tools/navigate', async (req, res) => {
  await executeWithContext('navigate', async (page, input, requestId) => {
    const { url, wait_for_load = true, timeout = DEFAULT_TIMEOUT } = input;
...
app.post('/tools/click', async (req, res) => {
  await executeWithContext('click', async (page, input, requestId) => {
    const { selector, timeout = DEFAULT_TIMEOUT } = input;
...
app.post('/tools/fill', async (req, res) => {
  await executeWithContext('fill', async (page, input, requestId) => {
    const { selector, value, timeout = DEFAULT_TIMEOUT } = input;
...
app.post('/tools/wait-for-selector', async (req, res) => {
  await executeWithContext('wait-for-selector', async (page, input, requestId) => {
    const { selector, timeout = DEFAULT_TIMEOUT, state = 'visible' } = input;

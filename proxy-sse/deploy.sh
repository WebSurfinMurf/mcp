...
if docker ps | grep -q ${CONTAINER_NAME}; then
    ...
    echo "========================================="
    echo ""
    echo "🔒 SECURITY: Network Isolated Deployment"
    echo "   - NO external port exposure (port 8585 removed)"
    echo "   - Only accessible via Docker network 'litellm-net'"
    echo ""
    echo "📡 Internal SSE Endpoints (for LiteLLM config):"
    echo ""
